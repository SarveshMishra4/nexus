...
interface CdUnitProps {
  value: string
  label: string
}

function CdUnit({ value, label }: CdUnitProps) {
  return (
...
export default function Home() {
  ...
  const [stats,   setStats  ] = useState<Stats>({ brands: 247, influencers: 1834, agencies: 89 })
  const [anim,    setAnim   ] = useState<Stats>({ brands: 0, influencers: 0, agencies: 0 })
  ...
  const launchConfetti = useCallback((): void => {
    ...
    const run = (): void => {
      ...
      confData.current.parts.forEach((p: ConfettiParticle) => {
        ...
        if (p.rect) {
          cc.fillRect(-p.size / 2, -p.size / 3.5, p.size, p.size / 2.8)
        } else {
          cc.beginPath()
          cc.ellipse(0, 0, p.size / 2, p.size / 3.5, 0, 0, Math.PI * 2)
          cc.fill()
        }
        cc.restore()
      })
  ...
  return (
    <>
      ...
      {/* Confetti canvas — separate from the map canvas */}
      <canvas
        ref={confCanvas}
      ...
      <main style={{ position: 'relative', zIndex: 1 }}>
        ...
        <section
          className="fu1"
          style={{ paddingTop: 'clamp(40px,6vw,72px)', paddingBottom: 0, paddingLeft: 16, paddingRight: 16, textAlign: 'center' }}
        >
          <div
            style={{
            ...
            {([
              ...
              >
                <span
                  className="block"
                  style={{
                    fontSize: 'clamp(28px,5vw,56px)', fontWeight: 900, lineHeight: 1, marginBottom: 8,
